...
//!
//! Port of `src/target.ts`. `sva shell <target>` takes whatever the person
...
//!
//! The input is the same joined view `sva machines` prints, so the CLI
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Unknown { argument, reachable } if reachable.is_empty() => write!(
                f,
                "No workspace called {argument}. You cannot reach any workspace yet; run `sva machines` to see what exists."
            ),
